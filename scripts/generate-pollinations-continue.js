#!/usr/bin/env node
/**
 * Continue Pollinations Image Generation Automation Script
 * Skips already generated images and continues from where we left off
 * 
 * Usage:
 *   export POLLINATIONS_API_KEY=sk_JNzA2U9q9lcGhUAPJBwk7pPL8bOCDlFK
 *   node generate-pollinations-continue.js [batch-name] [start-from]
 * 
 * Batches:
...
const SPECS = {
  hero: { width: 1024, height: 576 },  // 16:9 ratio
...
async function processBatch(batchKey, startIndex = 1) {
  ...
  console.log(`   Total images: ${batch.count}`);
  ...
  // Process prompts starting from startIndex (1-based)
  for (let i = startIndex - 1; i < prompts.length; i++) { // Convert to 0-based index
    const prompt = prompts[i].trim();
    if (!prompt) continue;
    
    // Calculate 1-based ID for display
    const displayId = i + 1;
    const id = `${batch.type}-${displayId}`;
    const filename = `${batch.type}/${batch.type}-${displayId}.jpg`;
    const outputPath = path.join(OUTPUT_BASE, filename);
    
    // Check if file already exists
    try {
      await fs.access(outputPath);
      console.log(`[${displayId}/${prompts.length}] ⏭️  Skipping (already exists): ${id}`);
      results.skipped.push({ id, path: filename });
      continue;
    } catch (err) {
      // File doesn't exist, proceed with generation
    }
    
    console.log(`[${displayId}/${prompts.length}] Generating: ${id}`);
...
async function main() {
  ...
  const startFrom = parseInt(process.argv[3]) || 1; // Default to start from 1
  
  console.log('╔════════════════════════════════════════════════════════╗');
  console.log('║   🎨 Paragu-AI Image Generation Automation             ║');
  console.log('║   Powered by Pollinations AI (Continue Version)        ║');
  console.log('╚════════════════════════════════════════════════════════╝');
  console.log('');
  ...
  for (const batchKey of batchesToProcess) {
    // For continue functionality, only heroes batch uses custom start index
    const startIndex = (batchKey === 'heroes') ? startFrom : 1;
    const results = await processBatch(batchKey, startIndex);
