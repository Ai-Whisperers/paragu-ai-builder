...
- Name: commerce-storefront.test.ts >> Viajero Comercio - Storefront >> search filters products
- Location: tests/e2e/commerce-storefront.test.ts:33:7
...
Locator: locator('input[type="search"]')
Expected: visible
Error: strict mode violation: locator('input[type="search"]') resolved to 2 elements:
    1) <input value="" type="search" autocomplete="off" id="header-search-viajero-comercio" placeholder="Buscas carpa, bolsa de dormir, equipo de pesca..." class="w-32 bg-transparent text-xs outline-none placeholder:text-[color:var(--text-muted,#9ca3af)] md:w-48"/> aka getByPlaceholder('Buscas carpa, bolsa de dormir')
    2) <input value="" type="search" placeholder="¿Qué buscás? (lubricante, vibrador, lencería…)" class="w-full bg-transparent text-sm outline-none placeholder:text-[color:var(--text-muted,#9ca3af)]"/> aka getByPlaceholder('¿Qué buscás? (lubricante,')
